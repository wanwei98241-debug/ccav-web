...
export default async function LessonDetailPage({ params }: Props) {
  ...
  // 关键概念卡片
              {(() => { const concepts = Array.isArray(lesson.keyConcept) ? lesson.keyConcept : [lesson.keyConcept]; return concepts.length > 0 && concepts.map((kc, i) => (
                <div key={i} className="p-5 rounded-xl bg-[#0d1117] border border-[#3fb950]/20 mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-[#3fb950]/15 flex items-center justify-center text-xl shrink-0">
                      {kc.icon || "💡"}
                    </div>
                    <div>
                      <h3 className="font-semibold text-[#f0f6fc] mb-1 flex items-center gap-2">
                        <Lightbulb className="w-4 h-4 text-[#3fb950]" />
                        {kc.title}
                      </h3>
                      <p className="text-sm text-[#8b949e] leading-relaxed">{kc.description}</p>
                    </div>
                  </div>
                </div>
              )); })()}
              ...
              {lesson.tryItPrompt && (
                <div className="p-5 rounded-xl bg-[#58a6ff]/5 border border-[#58a6ff]/20">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-semibold text-[#58a6ff] text-sm">先动手试试</span>
                  </div>
                  <p className="text-sm text-[#c9d1d9] mb-3">别等老师讲，现在就打开Playground跑一次：</p>
                  <div className="p-4 rounded-lg bg-[#0d1117] border border-[#30363d] font-mono text-sm text-[#c9d1d9] mb-3">
                    {lesson.tryItPrompt}
                  </div>
                  <a
                    href="/playground/"
                    target="_blank"
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-[#58a6ff] to-[#39d2c0] text-white text-sm font-semibold hover:shadow-lg transition-all"
                  >
              ...
              {lesson.tryItInternational && (
                <div className="mt-3 p-5 rounded-xl bg-[#d2991d]/5 border border-[#d2991d]/20">
                  ...
                  <p className="text-xs text-[#8b949e] mb-2">无法注册可灵/即梦？用以下国际工具替代：</p>
                  <div className="p-4 rounded-lg bg-[#0d1117] border border-[#30363d] font-mono text-sm text-[#c9d1d9]">
                    {lesson.tryItInternational}
                  </div>
                </div>
              )}
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-[#f0f6fc] mb-2">视频号直播 · 老师现场演示</h3>
                  <ul className="space-y-2 text-sm text-[#8b949e]">
                    <li>• 老师不念书，直接屏幕演示操作全流程</li>
                    <li>• 学员跟练 + 实时连麦提问</li>
                    <li>• 每节课产出一个小作品</li>
                  </ul>
                  <div className="mt-4 p-4 rounded-lg bg-[#0d1117] border border-[#30363d] text-center">
                    <p className="text-sm text-[#8b949e]">📅 直播排期即将公布</p>
                    <p className="text-xs text-[#484f58] mt-1">直播回放将在课后自动归档到本页</p>
                  </div>
                </div>
              </div>
            </LayerCard>
            ...
            >
              {/* 录播回放 */}
              <div className="p-4 rounded-lg bg-[#0d1117] border border-[#30363d] mb-4 flex items-center gap-3">
                <Play className="w-5 h-5 text-[#484f58]" />
                <span className="text-sm text-[#8b949e]">录播回放（课程上线后自动关联）</span>
              </div>

              {/* 课后作业 */}
              <div className="p-5 rounded-xl bg-[#0d1117] border border-[#d2991d]/20 mb-4">
                <h3 className="font-semibold text-[#f0f6fc] mb-2 flex items-center gap-2">
                  <Send className="w-4 h-4 text-[#d2991d]" />
                  课后作业
                </h3>
                <p className="text-sm text-[#c9d1d9] leading-relaxed">{lesson.homework}</p>
              </div>
              ...
              {lesson.homeworkInternational && (
                <div className="p-5 rounded-xl bg-[#d2991d]/5 border border-[#d2991d]/20 mb-4">
                  ...
                  <p className="text-sm text-[#c9d1d9] leading-relaxed">{lesson.homeworkInternational}</p>
                </div>
              )}

              {/* 进阶挑战 */}
              {lesson.advancedChallenge && (
                <div className="p-5 rounded-xl bg-[#0d1117] border border-[#e53e3e]/20 mb-4">
                  <h3 className="font-semibold text-[#f0f6fc] mb-2 flex items-center gap-2">
                    <Award className="w-4 h-4 text-[#e53e3e]" />
                    进阶挑战（选做）
                  </h3>
                  <p className="text-sm text-[#c9d1d9] leading-relaxed">{lesson.advancedChallenge}</p>
                </div>
              )}
              ...
              {lesson.nextPreview && (
                <div className="p-4 rounded-lg bg-[#0d1117] border border-[#30363d]">
                  <h3 className="text-xs text-[#484f58] uppercase mb-1">下节预告</h3>
                  <p className="text-sm text-[#8b949e]">{lesson.nextPreview}</p>
                </div>
              )}
            </LayerCard>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-[#21262d]">
              {prevLesson ? (
                <Link
                  href={prevLesson}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#161b22] border border-[#30363d] text-[#8b949e] hover:border-[#58a6ff]/50 hover:text-[#c9d1d9] text-sm transition-all"
                >
              ...
              {nextLesson ? (
                <Link
                  href={nextLesson}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-[#58a6ff] to-[#bc8cff] text-white text-sm font-semibold hover:shadow-lg transition-all"
                >
                  下一课
  ...
  return (
    <div className={`rounded-2xl border border-[#21262d] ${bg} border-l-4 ${color} overflow-hidden`}>
      <div className="px-6 py-4 border-b border-[#21262d]/50">
        <div className="flex items-center gap-3">
          <span className="w-8 h-8 rounded-lg bg-[#161b22] border border-[#30363d] flex items-center justify-center text-sm font-bold text-[#f0f6fc]">
            {number}
          </span>
          <div>
  ...
  return (
    <>
      ...
      <main className="flex-1 pt-32 text-center min-h-[60vh]">
        ...
        <Link href="/courses/" className="text-[#58a6ff] mt-4 inline-block">返回课程列表</Link>
      </main>
      <Footer />
    </>
  );
